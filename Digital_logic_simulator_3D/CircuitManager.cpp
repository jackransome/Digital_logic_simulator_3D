...
//initialises the CircuitManager object
...
void CircuitManager::addComponent(int x, int y, int z, componentType type) {
	addComponent(x, y, z, type, positiveX);
}
//adds a block to the vector
void CircuitManager::addComponent(int x, int y, int z, componentType type, componentDirection direction) {
...
int CircuitManager::getVectorSize() {
	return components.size();
}
//removes a block from the vector
bool CircuitManager::removeComponent(int x, int y, int z) {
	if (doesComponentExist(x, y, z)) {
...
bool CircuitManager::removeComponent(int _index)
{
...
bool CircuitManager::doesComponentExist(int x, int y, int z) {
...
int CircuitManager::getComponent(int x, int y, int z) {
	for (int i = 0; i < components.size(); i++) {
		if (components[i]->position == glm::vec3(x, y, z)) {
			return i;
		}
...
void CircuitManager::checkSurroundingComponents(glm::vec3 block) {
...
void CircuitManager::getInput(int blockIndex, glm::vec3* input)
{
...
void CircuitManager::getOutput(int blockIndex, glm::vec3* output)
{
	switch (components[blockIndex]->direction) {
	case positiveX:
...
void CircuitManager::getInputs(int blockIndex, glm::vec3* input1, glm::vec3* input2)
{
	switch (components[blockIndex]->direction) {
	case positiveX:
...
int CircuitManager::getModelIndex(componentType type, bool state)
{
	switch (type) {
	case inverter:
		return 8 + state;
...
//checks for differences between state and lastState and then updates components accordingly
void CircuitManager::checkDrawChanges(){

}

void CircuitManager::clearComponents() {
	for (int i = components.size() - 1; i >= 0; i--) {
		removeComponent(i);
	}
}

componentType CircuitManager::getComponentType(int _x, int _y, int _z) {
	if (doesComponentExist(_x, _y, _z)) {
		return components[getComponent(_x, _y, _z)]->type;
	}
}

bool CircuitManager::getComponentState(int _x, int _y, int _z) {
	if (doesComponentExist(_x, _y, _z)) {
		return components[getComponent(_x, _y, _z)]->state;
	}
}

void CircuitManager::setComponentState(int _x, int _y, int _z, bool _state) {
	if (doesComponentExist(_x, _y, _z)) {
		components[getComponent(_x, _y, _z)]->state = _state;
	}
}

std::vector<Component*>* CircuitManager::getComponentsPointer() {
	return &components;
}
